      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div>
          <motion.div
            ...
            viewport={{ once: true, margin: '-60px' }}
            variants={stagger}
            className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-px bg-border/40 rounded-2xl overflow-hidden border border-border/40"
          >
            {steps.map((step, idx) => (
              <motion.div
                key={step.number}
                variants={fadeUp}
                className="group bg-cream p-8 flex flex-col items-start hover:bg-white transition-colors duration-500 relative"
              >
                {/* Step number */}
                <span className="font-body text-[10px] font-semibold text-muted/40 uppercase tracking-[0.2em] mb-6">
                  {step.number}
                </span>

                {/* Icon with hover ring */}
                <div className="relative mb-6">
                  <div className="w-11 h-11 rounded-xl bg-soft text-muted flex items-center justify-center transition-all duration-500 group-hover:bg-brown group-hover:text-cream">
                    {step.icon}
                  </div>
                  {/* Pulse ring on hover */}
                  <div className="absolute inset-0 rounded-xl border border-gold/0 group-hover:border-gold/30 transition-all duration-500 scale-100 group-hover:scale-110" />
                </div>

                {/* Title */}
                <h3 className="font-heading font-bold text-base text-brown mb-2 group-hover:text-gold transition-colors duration-300">
                  {step.title}
                </h3>
                ...
                {/* Bottom accent line */}
                <div className="absolute bottom-0 left-0 h-px w-0 bg-gold group-hover:w-full transition-all duration-700 ease-out" />
              </motion.div>
            ))}
          </motion.div>
        </div>
